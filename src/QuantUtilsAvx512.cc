...
void requantizeOutputProcessingGConvAvx512(
  ...
  __m512 multiplier_v = _mm512_set1_ps(r.A_scale * r.B_scale[quant_param_idx]);
  ...
  for (int i = block.row_start; i < block.row_start + block.row_size; ++i) {
    ...
    for (; j < block.col_start + ((block.col_size + VLEN - 1) / VLEN * VLEN);
      ...
      __m512 xf_v = _mm512_cvtepi32_ps(x_v);
      ...
      if (Q_GRAN == QuantizationGranularity::OUT_CHANNEL) {
        if (C_PER_G != 8) {
          multiplier_v = _mm512_loadu_ps(r.B_scale + j);
        } else {
          multiplier_v = _mm512_maskz_loadu_ps(mask, r.B_scale + j);
        }
        x_scaled_v = _mm512_mul_ps(
            xf_v, _mm512_mul_ps(_mm512_set1_ps(r.A_scale), multiplier_v));
      } else if (Q_GRAN == QuantizationGranularity::GROUP) {
        if (C_PER_G == 2) {
          multiplier_v = _mm512_moveldup_ps(_mm512_permutexvar_ps(
              permute_mask_v_g8,
              _mm512_castps256_ps512(
                  _mm256_loadu_ps(r.B_scale + quant_param_idx))));
        } else if (C_PER_G == 4) {
          multiplier_v = _mm512_permutexvar_ps(
              permute_mask_v_g4,
              _mm512_broadcast_f32x4(
                  _mm_loadu_ps(r.B_scale + quant_param_idx)));
        } else if (C_PER_G == 8) {
          multiplier_v = _mm512_set1_ps(r.B_scale[quant_param_idx]);
        } else {
          multiplier_v = _mm512_set1_ps(r.B_scale[quant_param_idx]);
        }
        x_scaled_v = _mm512_mul_ps(
      ...
      if (HAS_BIAS) {
        x_scaled_v = _mm512_add_ps(x_scaled_v, _mm512_loadu_ps(r.bias + j));
      }
      if (FUSE_RELU) {
        x_scaled_v = _mm512_max_ps(_mm512_setzero_ps(), x_scaled_v);
      }

      if (C_PER_G != 8) {
        _mm512_storeu_ps(out + i * ld_out + j, x_scaled_v);
      } else {
        _mm256_storeu_ps(
            out + i * ld_out + j, _mm512_castps512_ps256(x_scaled_v));
      }
    } // j loop vectorized
...
#define INSTANTIATE_BIAS_TYPE(A_SYM, B_SYM, Q_GRAN, BIAS, RELU)              \
...
#define INSTANTIATE_A_SYM(B_SYM, Q_GRAN, BIAS, RELU)     \
  INSTANTIATE_BIAS_TYPE(true, B_SYM, Q_GRAN, BIAS, RELU) \
  INSTANTIATE_BIAS_TYPE(false, B_SYM, Q_GRAN, BIAS, RELU)
