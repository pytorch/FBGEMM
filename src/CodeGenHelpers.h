...
inline constexpr bool is_avx512_any_v =
...
template <inst_set_t instSet>
inline constexpr bool is_avx512_vnni_v =
    instSet == inst_set_t::avx512_vnni ||
    instSet == inst_set_t::avx512_vnni_ymm;
...
void gen16BitVectorOne(x86::Emitter* a, T dest) {
  if constexpr (instSet == inst_set_t::avx2) {
    ...
    static_assert(
        is_avx512_any_v<instSet>, "unsupported instruction set for gen16BitVectorOne");
...
void emitLoadDWord(x86::Emitter* a, T dest, const x86::Mem& ptr) {
  if constexpr (instSet == inst_set_t::avx2) {
    ...
    static_assert(
        is_avx512_any_v<instSet>, "unsupported instruction set for emitLoadDWord");
...
void emitExtractHalfVector(
    ...
    int idx) {
  static_assert(
      is_avx512_any_v<instSet>, "Zmm->Ymm extract requires AVX512");
...
void emitExtractHalfVector(
  ...
  if constexpr (instSet == inst_set_t::avx2) {
    ...
    static_assert(
        is_avx512_any_v<instSet>, "unsupported instruction set for emitExtractHalfVector");
