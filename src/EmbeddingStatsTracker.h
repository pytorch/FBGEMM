 ...
 * variables:
 * - FBGEMM_STATS_ENABLE: Enables tracking.
...
class EmbeddingStatsTracker {
 public:
  ...
  struct AccessPatternEntry {
    ...
    DataType data_type; // Data type (e.g., "fp32", "fp16", "int8", "int4")
    ...
    std::string toString() const {
      return "dims=" + std::to_string(dims) + ",rows=" + std::to_string(rows) +
  ...
  struct AccessPatternEntryHash {
    ...
    std::size_t operator()(const AccessPatternEntry& key) const {
      ...
      std::size_t h3 = std::hash<int>{}(static_cast<int>(key.data_type));
      std::size_t h4 = std::hash<int64_t>{}(key.batch_size);
      std::size_t h5 = std::hash<int64_t>{}(key.bag_size);
   ...
   * @param data_type Data type used for storing embeddings
