    ...
    for (int64_t m = 0; m < output_size; ++m) {
      ...
      if constexpr (isOutput8bit) {
        ...
        if (scale_bias_last) {
          ...
          bias = *(reinterpret_cast<const float*>(scale_bias_addr + sizeof(float)));
        } else {
...
static bool ALWAYS_INLINE EmbeddingSpMDMNBit_autovec(
  ...
  for (int64_t m = 0; m < output_size; ++m) {
    ...
    for (; current < end; ++current) {
      ...
      float16 bias16 = *reinterpret_cast<const float16*>(scale_bias_addr + sizeof(float16));
  ...
  if constexpr (is8bit) {
    ...
    for (int m = 0; m < output_size; ++m) {
      ...
      for (; current < end; ++current) {
        ...
        float bias = *(reinterpret_cast<const float*>(scale_bias_addr + sizeof(float)));
        if (weights != nullptr) {
          float weight = *weights_addr++;
          scale *= weight;
          bias *= weight;
