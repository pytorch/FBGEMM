...
static inline void fill_output(
  ...
  for (int j = 0; j < block_size; ++j) {
    out[j] = convert_from_float_ref<OutType>(src[j]);
  }
}
...
static constexpr uint8x16_t idx[] = {
    // Precompute table indices for packing 4 uint8 per uint32x4 (zeros in
    // between lanes)
    // idx0: select bytes 0-3 → positions 0,4,8,12
    {
        ...
        255,
        255,
        255, // byte 0 to lane 0
        1,
        255,
        255,
        255, // byte 1 to lane 4
        2,
        255,
        255,
        255, // byte 2 to lane 8
        3,
        255,
        255,
        255 // byte 3 to lane 12
    },
    // idx1: bytes 4-7
...
static ALWAYS_INLINE void Dequant4bitToF32_neon(
    ...
    int64_t offset) {
  if (block_size >= 32) {
    // Main vectorized loop: processes 32 floats per iteration (16 input bytes)
    // using table lookup for nibble packing
    ...
    for (int m = 0; m < output_size; ++m) {
      ...
      if (weights != nullptr) {
        ...
        for (; j < block_size - (block_size % FBGEMM_VECTOR_WIDTH); ++j) {
          ...
          buf[j] = weight * convert_to_float_ref(*inptr);
        }
#endif
        for (; j < block_size; ++j) {
          const InType* inptr = input + input_stride * idx + j;
        ...
        for (; j < block_size - (block_size % FBGEMM_VECTOR_WIDTH); ++j) {
          const InType* inptr = input + input_stride * idx + j;
          buf[j] = convert_to_float_ref(*inptr);
        }
#endif
        for (; j < block_size; ++j) {
          const InType* inptr = input + input_stride * idx + j;
  ...
  for (int m = 0; m < output_size; ++m) {
    ...
    for (int i = 0; i < len; ++i) {
      ...
      for (; j < block_size - (block_size % FBGEMM_VECTOR_WIDTH); ++j) {
        ...
        buf[j] = std::fma(w, convert_to_float_ref(value), buf[j]);
      }
#endif
      for (; j < block_size; ++j) {
        InType value = *input_row++;
