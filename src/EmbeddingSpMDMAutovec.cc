    ...
    return [=](int64_t output_size,                                       \
      ...
      if (std::is_same<InType, uint8_t>::value) {                         \
        ...
        return EmbeddingSpMDM_autovec(                                    \
            ...
            specialize(NO_BAG, no_bag),                                   \
            specialize(IS_BF16_OUT, is_bf16_out),                         \
            specialize(IS_BF16_IN, is_bf16_in));                          \
      }                                                                   \
    };                                                                    \
  }
...
#define SPECIALIZE_BLOCK_SIZE(                                             \
  ...
  SPECIALIZE(                                                              \
      ...
      USE_OFFSETS,                                                         \
      /*OUTPUT_STRIDE*/ var,                                               \
      /*INPUT_STRIDE*/ fixed(stride_SpMDMWithStrides<InType>(254, false)), \
      /*SCALE_BIAS_LAST*/ fixed(false),                                    \
      NO_BAG,                                                              \
      IS_BF16_OUT,                                                         \
      IS_BF16_IN)                                                          \
  ...
  /* The last specialization should always match and we should not get here. */
  abort();
  ...
  if (match(INPUT_BIT_RATE, input_bit_rate) &&                          \
    ...
    return [=](int64_t output_size,                                     \
      ...
      return EmbeddingSpMDMNBit_autovec(                                \
          specialize(INPUT_BIT_RATE, input_bit_rate),                   \
          specialize(BLOCK_SIZE, block_size),                           \
          output_size,                                                  \
          index_size,                                                   \
          data_size,                                                    \
          ...
          specialize(IS_BF16_OUT, is_bf16_out),                         \
          specialize(NO_BAG, no_bag),                                   \
          specialize(OUTPUT_BIT_RATE, output_bit_rate));                \
    };                                                                  \
  }
...
#define SPECIALIZE_INPUT_RATE(     \
  ...
  SPECIALIZE(
      ...
      var,
      var);
  /* Should always match the last sepcializations and never get here. */
  abort();
