...
PackEncodedBMatrix<T, accT>::PackEncodedBMatrix(
    matrix_op_t trans,
    int32_t nRow,
    int32_t nCol,
    const T* smat,
    ...
    : PackMatrix<PackEncodedBMatrix<T, accT>, T, accT>(
          nRow,
          nCol + 1,
          pmat,
          groups,
...
const inst_set_t isa = fbgemmInstructionSet();
  ...
  // actual number of cols of B;
  actual_n_cols_ = BaseType::numCols() - 1;
  pack(block, params);
}

template <typename T, typename accT>
void PackEncodedBMatrix<T, accT>::pack_unpack_(
  ...
  for (int g = 0; g < BaseType::numGroups(); ++g) {
    ...
    for (int i = block.row_start; i < block.row_start + block.row_size; ++i) {
      ...
      for (int j = block.col_start; j < block.col_start + block.col_size; ++j) {
        ...
        if (ispack) {
          // accumulate row sum if j is less than actuanl number of cols
          if (j < actual_n_cols_) {
            // block.col_size - 1 because it's actual number of cols
            T pack_element = tr
                ? unpack_buf[i + (g * (block.col_size - 1) + j) * ld_]
                : unpack_buf[(g * block.row_size + i) * ld_ + j];
            pack_buf_cur[r_offset + c_offset] = pack_element;
            row_sum_abft += static_cast<int32_t>(pack_element);
          } else { // row sum calculation is done. so pack into pack buffer
            pack_buf_cur[r_offset + c_offset] =
                static_cast<T>(row_sum_abft % MOD_Int8);
          }
        } else {
          if (j < actual_n_cols_) {
            T* unpack_buf_cur = tr
...
template <typename T, typename accT>
void PackEncodedBMatrix<T, accT>::pack(
    const block_type_t& block,
    const BlockingFactors* params) {
...
template <typename T, typename accT>
void PackEncodedBMatrix<T, accT>::unpack(
    T* origin_buf,
    const BlockingFactors* params) {
...
template <typename T, typename accT>
int32_t PackEncodedBMatrix<T, accT>::addr(int32_t r, int32_t c) const {
...
template <typename T, typename accT>
void PackEncodedBMatrix<T, accT>::printPackedMatrix(
    std::string name,
    const BlockingFactors* params) {
  std::cout << name << ":"
            << "[" << BaseType::numPackedRows() << ", "
...
template <typename T, typename accT>
bool PackEncodedBMatrix<T, accT>::metaEquals(
    const PackEncodedBMatrix<T, accT>& that) const {
...
template <typename T, typename accT>
bool PackEncodedBMatrix<T, accT>::equals(
    const PackEncodedBMatrix<T, accT>& that) const {
  if (!metaEquals(that)) {
    return false;
  }
...
template class PackEncodedBMatrix<int8_t, int32_t>;
template class PackEncodedBMatrix<int8_t, int16_t>;
} // namespace fbgemm
