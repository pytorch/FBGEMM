...
void ExecuteKernel<
  ...
  for (int jb = jb_begin; jb < jb_end; ++jb) {
    ...
    if (lastKBlock && jb == jb_end - 1) {
      ...
      if (!packedA_
               .isPackingEncodedA()) { // if packA doesn't contain encoded row
        int32_t nSize =
        ...
        if (nSize) {
          if (fbgemmHasAvx512Support()) {
            ...
            // TODO: Have default slower path
            assert(0 && "unsupported architecure");
            throw std::runtime_error("unsupoorted architecture");
          }
          // accumulate C's column sum as int64 before requantized
          if (packedB_.isInt32AccForABFTEnabled()) {
            int64_t* cSum = packedB_.getCColSumBuf();
            int32_t* abft_inp = C_buffer_row_start + jb_begin * nbSize_;
            int32_t abft_ldIn = ldc_;
            int32_t abft_row_start = row_start_A;
            int32_t abft_row_size = packed_rows_A;
            int32_t abft_col_start = NDim * group + jb_begin * nbSize_;
            int32_t abft_col_size = nSize;
            for (int32_t i = abft_row_start; i < abft_row_start + abft_row_size;
                 i++) {
              for (int32_t j = abft_col_start;
                   j < abft_col_start + abft_col_size;
                   j++) {
                int32_t abft_c_inbuffer = abft_inp
                    [(i - abft_row_start) * abft_ldIn + (j - abft_col_start)];
                cSum[j] += abft_c_inbuffer;
              }
            }
          } // end ABFT accumulation
        }
        ...
        if (C_buffer_start == C_tile_) {
          ...
          // accumulate cSum as int64 before requantization
          if (packedB_.isInt32AccForABFTEnabled()) {
            int64_t* cSum = packedB_.getCColSumBuf();
            int32_t* abft_inp = C_tile_;
            int32_t abft_ldIn = leadingDim;
            int32_t abft_row_start = row_start_A;
            int32_t abft_row_size = packed_rows_A;
            int32_t abft_col_start = NDim * group + jb * nbSize_;
            int32_t abft_col_size = packedB_.lastBcol();
            for (int32_t i = abft_row_start; i < abft_row_start + abft_row_size;
                 i++) {
              for (int32_t j = abft_col_start;
                   j < abft_col_start + abft_col_size;
      ...
      } else { // if packA contains encoded row
        int32_t nSize =
            (C_buffer_start == C_tile_ ? (jb - jb_begin) * nbSize_
                                       : (jb_end - jb_begin) * nbSize_);
        // actual number of rows in A is 1 less than encodedA
        int32_t actual_nrows_packedA = packedA_.numRows() - 1;
        int32_t actual_rowsize_packedA =
            std::min(actual_nrows_packedA - row_start_A, packed_rows_A);
        if (nSize) { // nSize is the columns not including C_tile
          if (fbgemmHasAvx512Support()) {
            // TODO: avx512 path
            // Currently use avx2 code
            outputProcess_.template f<inst_set_t::avx2>(
          ...
          int32_t abft_ldIn = ldc_;
          int32_t abft_row_start = row_start_A;
          int32_t abft_row_size = actual_rowsize_packedA;
          int32_t abft_col_start = NDim * group + jb_begin * nbSize_;
          int32_t abft_col_size = nSize;
          for (int32_t i = abft_row_start; i < abft_row_start + abft_row_size;
               i++) {
            for (int32_t j = abft_col_start; j < abft_col_start + abft_col_size;
                 j++) {
              int32_t abft_c_inbuffer = abft_inp
                  [(i - abft_row_start) * abft_ldIn + (j - abft_col_start)];
              cSum[j] += abft_c_inbuffer;
          ...
          // start ABFT verification: if this is the last row of block
          if (abft_row_start + packed_rows_A == packedA_.numRows()) {
            int errCnt = 0;
            for (int j = abft_col_start; j < abft_col_start + abft_col_size;
                 j++) {
              if ((cSum[j] -
                   abft_inp[(abft_row_size)*abft_ldIn + (j - abft_col_start)]) %
                      MOD_Uint8 !=
                  0) {
                errCnt++;
              }
            ...
            if (errCnt > 0) {
              std::cerr << "!!!!!!!!!!!!!silent error detected stage 1: "
                        << errCnt << std::endl;
            }
          } // end ABFT verification
        }

        if (C_buffer_start ==
            C_tile_) { // nSize is not equal to B's number of columns
          // When C_tile_ scratchpad was used to avoid accessing memory past
          // C_buffer_ .
          if (fbgemmHasAvx512Support()) {
            // TODO: avx512 path
          ...
          if (abft_row_start + packed_rows_A == packedA_.numRows()) {
            int errCnt = 0;
            for (int j = abft_col_start; j < packedB_.numCols(); j++) {
              if ((cSum[j] -
                   abft_inp[(abft_row_size)*abft_ldIn + (j - abft_col_start)]) %
                      MOD_Uint8 !=
                  0) {
            ...
            if (errCnt > 0) {
              std::cerr << "!!!!!!!!!!!!!silent error detected stage 2: "
                        << errCnt << std::endl;
            }
          } // end ABFT verification
        } // end if: C_buffer_start == C_tile_
      } // end if: packA contains encoded checksum row
...
// New execute kernel function for PackEncodedBMatrix class
template <typename packingAMatrix, typename cT, typename processOutputType>
ExecuteKernel<
    packingAMatrix,
    PackEncodedBMatrix<int8_t, typename packingAMatrix::accType>,
    cT,
    processOutputType>::
    ExecuteKernel(
        PackMatrix<packingAMatrix, uint8_t, typename packingAMatrix::accType>&
  ...
  for (int jb = jb_begin; jb < jb_end; ++jb) {
    ...
    if (lastKBlock && jb == jb_end - 1) {
      ...
      int32_t nSize =
          (C_buffer_start == C_tile_ ? (jb - jb_begin) * nbSize_
                                     : (jb_end - jb_begin) * nbSize_);
      // tmporary sapce for C's row sum
      int64_t* c_row_sum = new int64_t[packed_rows_A]();

      if (nSize) {
        // variable to count number of wrong rows
        int32_t abft_errcnt = 0;
        // accumulate C's column sum as int64 before requantized
        int32_t* abft_inp = C_buffer_row_start + jb_begin * nbSize_;
        int32_t abft_ldIn = ldc_;
        int32_t abft_row_start = row_start_A;
        ...
        for (int32_t i = abft_row_start; i < abft_row_start + abft_row_size;
             i++) {
          int32_t j = abft_col_start;
          // exclude the encoded column when accumulate row sum of C_buffer
          for (j = abft_col_start; j <
               std::min(abft_col_start + abft_col_size, (group + 1) * NDim - 1);
               j++) {
            int32_t abft_c_inbuffer = abft_inp
                [(i - abft_row_start) * abft_ldIn + (j - abft_col_start)];
            c_row_sum[i - abft_row_start] += abft_c_inbuffer;
          }
          // this is the condition when row sum is accumulated completely
          if (abft_col_start + abft_col_size >= (group + 1) * NDim &&
              j == (group + 1) * NDim - 1) {
            if ((c_row_sum[i - abft_row_start] -
                 abft_inp
                     [(i - abft_row_start) * abft_ldIn +
                      (j - abft_col_start)]) %
                    MOD_Int8 !=
                0) {
              abft_errcnt++;
        ...
        if (abft_errcnt > 0) {
          std::cout
              << "!!!!!!silent error detected. number of wrong rows in C: "
              << abft_errcnt << std::endl;
        }
        // skip the output processing for ABFT encoded column
        if (std::min(
                nSize,
                (group + 1) * NDim - 1 - (NDim * group + jb_begin * nbSize_)) >
            0) {
          if (fbgemmHasAvx512Support()) {
            // TODO: avx512 path
            // Currently use avx2 code
            // keep this for future debugging
            // std::cout << "ldc_: " << ldc_ << std::endl;
            // std::cout<< "num of groups: " << packedB_.numGroups() <<
            // std::endl; std::cout << "nSize: " << nSize << std::endl;
            // std::cout <<"min size: " << (group+1)*NDim - 1 - (NDim * group +
            // jb_begin * nbSize_) << std::endl; std::cout<< "current group: "
            // << group<< std::endl; std::cout << "row start, size, col start,
            // size: " << row_start_A << " " << packed_rows_A << " " <<NDim *
            // group + jb_begin * nbSize_ << " " << std::min(nSize,
            // (group+1)*NDim - 1 - (NDim * group + jb_begin * nbSize_)) <<
            // std::endl;
            outputProcess_.template f<inst_set_t::avx2>(
                matC_,
                ...
                {row_start_A,
                 packed_rows_A,
                 NDim * group + jb_begin * nbSize_ -
                     group, // -group for offset because of the encoded column
                 std::min(
                     nSize,
                     (group + 1) * NDim - 1 -
                         (NDim * group +
                          jb_begin * nbSize_))}, // don't include encoded col
                                                 // for output processing
                ldc_ - packedB_.numGroups(), // -number of groups for matC's
                                             // leading dimension
                ldc_); // incremented ld for C_buffer because of encoded column
          } else {
            // TODO: Have default slower path
            assert(0 && "unsupported architecure");
            throw std::runtime_error("unsupported architecure");
      ...
      if (C_buffer_start == C_tile_) {
        ...
        if (abft_errcnt > 0) {
          std::cout << "!!!!!!silent error detected. number of wrong rows: "
                    << abft_errcnt << std::endl;
        }
        if (std::min(
                packedB_.lastBcol(),
                (group + 1) * NDim - 1 - (NDim * group + jb * nbSize_)) > 0) {
          // When C_tile_ scratchpad was used to avoid accessing memory past
          // C_buffer_ .
          if (fbgemmHasAvx512Support()) {
            // TODO: avx512 path
            ...
            outputProcess_.template f<inst_set_t::avx2>(
                ...
                {row_start_A,
                 packed_rows_A,
                 NDim * group + jb * nbSize_ - group,
                 std::min(
                     packedB_.lastBcol(),
                     (group + 1) * NDim - 1 - (NDim * group + jb * nbSize_))},
                ldc_ - packedB_.numGroups(),
                leadingDim);
          } else {
            // TODO: Have default slower path
            assert(0 && "unsupported architecure");
...
INSTANTIATE_REQUANT_ACC_T(PackEncodedAWithRowOffset);
...
INSTANTIATE_REQUANT_FLOAT_RELU(PackEncodedAWithRowOffset);
...
INSTANTIATE_SPMDM_RELU(PackEncodedAWithRowOffset);
...
INSTANTIATE_MEMCPY_ACC_T(PackEncodedAWithRowOffset);
...
#define INSTANTIATE_REQUANT_BASE(PACK_A, ACC_T, RELU, Q_GRAN, BIAS_TYPE) \
  template class ExecuteKernel<                                          \
      PACK_A<uint8_t, ACC_T>,                                            \
      PackEncodedBMatrix<int8_t, ACC_T>,                                 \
      uint8_t,                                                           \
      ReQuantizeOutput<RELU, Q_GRAN, BIAS_TYPE>>;
...
#define INSTANTIATE_REQUANT_FLOAT_BASE(PACK_A, RELU, Q_GRAN) \
  template class ExecuteKernel<                              \
      PACK_A<uint8_t, int32_t>,                              \
      PackEncodedBMatrix<int8_t, int32_t>,                   \
      float,                                                 \
      ReQuantizeForFloat<RELU, Q_GRAN>>;
...
template class ExecuteKernel<
    PackAWithRowOffset<uint8_t, int16_t>,
    PackEncodedBMatrix<int8_t, int16_t>,
    float,
    ReQuantizeForFloat<false /* FUSE_RELU*/>>;
