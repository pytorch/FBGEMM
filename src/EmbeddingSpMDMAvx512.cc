...
void compressed_indices_remap_avx512(
  ...
  for (int k = 1; k < offsets_len; ++k) {
    int out_len = out_offsets[k] - out_offsets[k - 1];
    mymemcpy(
        reinterpret_cast<char*>(out_indices + offsets[k - 1]),
        reinterpret_cast<char*>(out_indices + out_offsets[k - 1]),
        out_len * sizeof(IndexType));
    if (HAS_WEIGHTS) {
      mymemcpy(
          reinterpret_cast<char*>(out_weights + offsets[k - 1]),
          reinterpret_cast<char*>(out_weights + out_offsets[k - 1]),
