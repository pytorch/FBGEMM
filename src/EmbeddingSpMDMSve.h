          ...
          for (; tile_current < end; ++tile_current) {
            ...
            if (weights != nullptr) {
              scale_val = static_cast<float16_t>(
                  static_cast<float>(scale_val) * *tile_weights);
              bias_val = static_cast<float16_t>(
                  static_cast<float>(bias_val) * *tile_weights);
              ++tile_weights;
            }
    ...
    for (; current < end; ++current) {
      ...
      if (block_size_mod > 0) {
        ...
        for (uint64_t k = 0; k < block_size_mod; ++k) {
          if (bit_rate == 4) {
            unpacked[k] =
                (input_row[iters * bytes_per_8elem + k / 2] >> ((k % 2) * 4)) &
                0x0f;
          } else {
            unpacked[k] =
                (input_row[iters * bytes_per_8elem + k / 4] >> ((k % 4) * 2)) &
                0x03;
          }
        }
        svuint16_t in_v = svld1ub_u16(lastPred16, unpacked);
        svfloat16_t in_v_f = svcvt_f16_u16_x(lastPred16, in_v);
