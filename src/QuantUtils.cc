...
void FusedNBitRowwiseQuantizedSBHalfRef(
    int bit_rate,
    const uint8_t* input,
    int input_rows,
    int input_columns,
  ...
  for (std::size_t row = 0; row < input_rows; ++row) {
    ...
    for (std::size_t col = 0; col < output_columns; ++col) {
      ...
      float output = scale * quantized + bias;
      if (std::is_same<OutputType, float>()) {
        output_row[col] = output;
      } else {
        output_row[col] = cpu_float2half_rn(output);
      }
    }
  }
}
...
template <typename OutputType>
void FusedNBitRowwiseQuantizedSBHalf(
    int bit_rate,
    const uint8_t* input,
    int input_rows,
    int input_columns,
  ...
  if (cpuinfo_initialize() && fbgemmHasAvx2Support()) {
    switch (bit_rate) {
      case 2:
        FusedNBitRowwiseQuantizedSBHalfAvx2<OutputType, 2>(
            input, input_rows, input_columns, output);
        break;
      case 4:
        FusedNBitRowwiseQuantizedSBHalfAvx2<OutputType, 4>(
            input, input_rows, input_columns, output);
        break;
      case 8:
        FusedNBitRowwiseQuantizedSBHalfAvx2<OutputType, 8>(
            input, input_rows, input_columns, output);
        break;
      default:
        FusedNBitRowwiseQuantizedSBHalfRef<OutputType>(
            bit_rate, input, input_rows, input_columns, output);
    }
  } else {
    FusedNBitRowwiseQuantizedSBHalfRef<OutputType>(
...
void FusedNBitRowwiseQuantizedSBHalfToFloat(
    ...
    int input_columns,
    float* output) {
  FusedNBitRowwiseQuantizedSBHalf<float>(bit_rate, input, input_rows, input_columns, output);
}
...
#define INSTANTIATE_QuantizationFunctions(type)                        \
  ...
  template FBGEMM_API void ToFusedNBitRowwiseQuantizedSBHalf<type>(    \
      ...
      int input_columns,                                               \
      std::uint8_t* output);                                           \
  template FBGEMM_API void FusedNBitRowwiseQuantizedSBHalfRef<type>(   \
      int bit_rate, \
      const uint8_t* input,\
      int input_rows,\
      int input_columns,\
      type* output); \
  template FBGEMM_API void FusedNBitRowwiseQuantizedSBHalf<type>(   \
      int bit_rate, \
      const uint8_t* input,\
      int input_rows,\
      int input_columns,\
