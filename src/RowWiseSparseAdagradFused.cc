...
class GenRowWiseSparseAdagradFused {
 public:
  ...
  typename ReturnFunctionSignature<indxType, offsetType, dataType>::
      ...
      getOrCreate(const int* mask_avx2, int block_size, int prefetch,
                  bool use_offsets, bool use_stochastic_rounding);
...
typename ReturnFunctionSignature<indxType, offsetType, dataType>::
  ...
  tuple<const int*, int, int, bool, bool> kernelSig =
      make_tuple(mask_avx2, block_size, prefetch, use_offsets,
      ...
      [&]() -> typename ReturnFunctionSignature<
        ...
        if (areWeightsFp16 && use_stochastic_rounding) {
          ...
          if (instSet == inst_set_t::avx2) {
            ...
            a->vmovdqa(S1_vreg.ymm(), x86::dword_ptr(rand_buffer,
                                                 1 * vlen * sizeof(uint32_t)));
            a->vmovdqa(S2_vreg.ymm(), x86::dword_ptr(rand_buffer,
                                                 2 * vlen * sizeof(uint32_t)));
            a->vmovdqa(S3_vreg.ymm(), x86::dword_ptr(rand_buffer,
                                                 3 * vlen * sizeof(uint32_t)));
          } else { // AVX512
            a->vmovdqa32(S0_vreg, x86::dword_ptr(rand_buffer));
            a->vmovdqa32(S1_vreg, x86::dword_ptr(rand_buffer,
                                                 1 * vlen * sizeof(uint32_t)));
            a->vmovdqa32(S2_vreg, x86::dword_ptr(rand_buffer,
                                                 2 * vlen * sizeof(uint32_t)));
            a->vmovdqa32(S3_vreg, x86::dword_ptr(rand_buffer,
                                                 3 * vlen * sizeof(uint32_t)));
          }
        }
          ...
          for (int v = 0; v < cur_unroll_factor; ++v) {
            ...
            if (!areWeightsFp16) { // float weights
              auto w_ptr = x86::dword_ptr(
                  w, scratchReg1, 2,
                  (vec_idx + v) * vlen * sizeof(dataType));
            ...
            if (prefetch && (vec_idx + v) % VLOAD_PER_CACHE_LINE == 0) {
              a->prefetchw(x86::dword_ptr(
                  w, scratchReg2, areWeightsFp16 ? 1 : 2,
        ...
        if (areWeightsFp16 && use_stochastic_rounding) {
          if (instSet == inst_set_t::avx2) {
            ...
            a->vmovdqa(x86::dword_ptr(rand_buffer,
                                  1 * vlen * sizeof(uint32_t)), S1_vreg.ymm());
            a->vmovdqa(x86::dword_ptr(rand_buffer,
                                  2 * vlen * sizeof(uint32_t)), S2_vreg.ymm());
            a->vmovdqa(x86::dword_ptr(rand_buffer,
                                  3 * vlen * sizeof(uint32_t)), S3_vreg.ymm());
          } else {
            a->vmovdqa32(x86::dword_ptr(rand_buffer), S0_vreg);
            a->vmovdqa32(x86::dword_ptr(rand_buffer,
                                  1 * vlen * sizeof(uint32_t)), S1_vreg);
            a->vmovdqa32(x86::dword_ptr(rand_buffer,
                                  2 * vlen * sizeof(uint32_t)), S2_vreg);
            a->vmovdqa32(x86::dword_ptr(rand_buffer,
...
FBGEMM_API
  ...
  if (fbgemmHasAvx512Support() || fbgemmHasAvx2Support()) {
    ...
    static GenRowWiseSparseAdagradFused<IndexType, OffsetType, DataType,
                                        inst_set_t::avx2> kernel_generator;
    const auto original_func =
        kernel_generator.getOrCreate(internal::avx2_ps_or_epi32_combined_mask,
                                     block_size,
                                     prefetch,
                                     use_offsets,
                                     use_stochastic_rounding);
    const auto lambda_func = [=](int64_t output_size,
      ...
      // TODO: JIT
      if (std::is_same<DataType, float16>::value) {
