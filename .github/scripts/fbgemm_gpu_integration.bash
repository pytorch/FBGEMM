...
integration_setup_conda_environment () {
  ...
  echo "[INTEGRATION] Creating the Conda build environment: ${env_name} ..."
  ...
  echo "[INTEGRATION] Will install PyTorch (version, variant): (${pytorch_variant_type}, ${pytorch_variant_version})"
  ...
  if [ "$pytorch_variant_type" == "cuda" ] || [ "$pytorch_variant_type" == "genai" ]; then
    ...
    print_exec conda env config vars set -n "${env_name}" BUILD_ROCM_VERSION="${pytorch_variant_version}"
  fi
