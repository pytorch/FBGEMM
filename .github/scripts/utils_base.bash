...
reset_cxx_symlinks () {
  ...
  if [ "$env_name" == "" ]; then
    ...
    echo "Example(s):"
    echo "    ${FUNCNAME[0]} build_env    # Reset CC and CXX for the build_env environment"
    return 1
  else
    echo "################################################################################"
    echo "# Reset CXX Symlinks"
    echo "#"
  ...
  if conda run ${env_prefix} which clang; then
    # shellcheck disable=SC2155,SC2086
    ...
    # shellcheck disable=SC2155,SC2086
    local cc_path=$(conda run ${env_prefix} which gcc)
    # shellcheck disable=SC2155,SC2086
    local cxx_path=$(conda run ${env_prefix} which g++)
  fi

  echo "[BUILD] Current CC and CXX values:"
  # shellcheck disable=SC2086
  print_exec conda run --no-capture-output ${env_prefix} printenv CC
  # shellcheck disable=SC2086
  print_exec conda run --no-capture-output ${env_prefix} printenv CXX

  echo "[BUILD] Resetting CC and CXX as needed ..."
  # shellcheck disable=SC2086
  print_exec conda env config vars set ${env_prefix} CC="${cc_path}"
  # shellcheck disable=SC2086
  print_exec conda env config vars set ${env_prefix} CXX="${cxx_path}"

  echo "[BUILD] Updated CC and CXX values:"
  # shellcheck disable=SC2086
  print_exec conda run --no-capture-output ${env_prefix} printenv CC
  # shellcheck disable=SC2086
  print_exec conda run --no-capture-output ${env_prefix} printenv CXX
