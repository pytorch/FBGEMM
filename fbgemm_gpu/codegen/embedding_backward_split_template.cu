...
split_embedding{{ "_nobag" if nobag else "" }}_backward_codegen_{{ optimizer }}_{{ wdesc }}_kernel_cta_per_row_1(
    ...
    {% else %}
    const PackedTensorAccessor32<int64_t, 1, RestrictPtrTraits> sorted_infos,
    {% endif %}
    {% if not dense %}
    const PackedTensorAccessor32<int32_t, 1, RestrictPtrTraits>
  ...
  for (int32_t long_run_id = blockIdx.x; long_run_id < num_long_runs; long_run_id += gridDim.x) {
        ...
        #pragma unroll kMaxVecsPerThread
                for (int32_t i = 0;
                    ...
                    {% else %}
                    Vec4T<acc_type<cache_t, true>> grad_out_vec(&grad_output[l_j][d]);
                    {% endif %}
                    {% if weighted %}
...
{{ "void" if not dense else "Tensor" }} split_embedding{{ "_nobag" if nobag else "" }}_backward_codegen_{{ optimizer }}_{{ wdesc }}_exact_cuda(
    ...
    {% else %}
    auto infos = at::empty_like(indices, indices.options().dtype(kLong));
    {% endif %}
...
__global__ void
__launch_bounds__(kMaxThreads)
split_embedding_nobag_backward_codegen_{{ optimizer }}_kernel_cta_per_row_1(
    const PackedTensorAccessor32<acc_type<cache_t, true>, 2, RestrictPtrTraits>
        grad_output,
    PackedTensorAccessor64<emb_t, 1, RestrictPtrTraits> dev_weights,
    {% if not dense %}
...
__launch_bounds__(kBackwardMaxThreads)
void
split_embedding_nobag_backward_codegen_{{ optimizer }}_kernel_warp_per_row_1(
    const PackedTensorAccessor32<acc_type<cache_t,true>, 2, RestrictPtrTraits>
        grad_output,
    PackedTensorAccessor64<emb_t, 1, RestrictPtrTraits> dev_weights,
    {% if not dense %}
...
{{ "void" if not dense else "Tensor" }} split_embedding_nobag_backward_codegen_{{ optimizer }}_exact_cuda(
    Tensor grad_output,
    Tensor dev_weights,
    {% if not dense %}
    Tensor uvm_weights,
    ...
    {% endif %}
        ...
        lxu_cache_weights.type(),
        {% endif %}
        "split_embedding_nobag_backward_{{ optimizer }}_exact_kernel",
        ([&] {
            auto grad_output_accessor = grad_output.packed_accessor32<
                acc_type<{{ "scalar_t" if dense else "cache_t" }}, true>,
                2,
