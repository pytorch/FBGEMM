...
void split_embedding_nobag_backward_exact_cpu_kernel(
  ...
  std::unordered_map<index_t, std::unordered_map<index_t, std::vector<at::acc_type<grad_t, true>>>> idx_to_table_grad_map;
  std::unordered_map<index_t, std::unordered_map<index_t, int64_t>> idx_to_table_feature_begin;
  ...
  for (const auto t : c10::irange(T)) {
    ...
    for (const auto b : c10::irange(B)) {
      ...
      for (auto i = indices_start; i < indices_end; ++i) {
        ...
        auto& grad_buffer = idx_to_table_grad_map[idx][table_begin];
        idx_to_table_feature_begin[idx][table_begin] = feature_begin;
  ...
  idx_vec.reserve(idx_to_table_grad_map.size());
  for (const auto& [idx, _] : idx_to_table_grad_map) {
  ...
  at::parallel_for(0, idx_vec.size(), 0, [&](int64_t start_idx, int64_t end_idx) {
    for (int64_t i = start_idx; i < end_idx; ++i) {
      ...
      const auto& grad_buffer_ = idx_to_table_grad_map[idx];
      
      for (const auto& [table_begin, grad_buffer] : grad_buffer_) {
        [[maybe_unused]] const auto feature_begin = idx_to_table_feature_begin[idx][table_begin];
        [[maybe_unused]] const int64_t embedding_begin = table_begin + idx * D;
        {{ split_weight_update_cpu }}
      }
    }
