...
def ensemble_rowwise_adagrad() -> Dict[str, Any]:
    ...
    at::acc_type<cache_t, true> debias;
    if (threadIdx.x == 0) {
        at::acc_type<cache_t, true> new_sum_square_grads = momentum2[idx] + g_avg_square;
        momentum2[idx] = new_sum_square_grads;
        multiplier = learning_rate / (sqrtf(new_sum_square_grads) + eps);
        row_counter[idx] += 1.0;
        should_ema = (fmod(row_counter[idx], step_ema) < 0.5);   
        should_swap = (fmod(row_counter[idx], step_swap) < 0.5);   
        debias  = 1.0 - powf(momentum, row_counter[idx] / step_ema);  
    }    
    multiplier = SHFL_SYNC(multiplier, 0);
    should_ema = SHFL_SYNC(should_ema, 0);
    should_swap = SHFL_SYNC(should_swap, 0);
    debias = SHFL_SYNC(debias, 0);
    """
    ...
    split_weight_update = """
        ...
        if (should_swap) { // slow-to-fast swap
            Vec4T<momentum1_ph_t> m_t(&momentum1[idx * D + d]);
            weight_new.acc.x = m_t.acc.x / debias;
            weight_new.acc.y = m_t.acc.y / debias;
            weight_new.acc.z = m_t.acc.z / debias;
            weight_new.acc.w = m_t.acc.w / debias;
        }
    """
