...
def generate(**kwargs: Any) -> None:
    ...
    if not kwargs.get("dense"):
        ...
        template_pt2 = env.get_template(
            "embedding_autograd_split_host_pt2_template.cpp"
        )
        filename_pt2 = f"gen_embedding_autograd_split_{optimizer}_pt2.cpp"
        write(filename_pt2, template_pt2.render(**kwargs))
        print(f"[Backward Split] [{optimizer}]: {filename_pt2}")

        # Generate PT2 backward wrapper
        template_pt2 = env.get_template("embedding_wrapper_split_host_pt2_template.cpp")
        filename_pt2 = f"gen_embedding_backward_split_wrapper_{optimizer}_pt2.cpp"
    ...
    if not kwargs.get("dense"):
        ...
        template_pt2 = env.get_template(
            "embedding_wrapper_split_host_cpu_pt2_template.cpp"
        )
        filename_pt2 = f"gen_embedding_backward_split_wrapper_{optimizer}_cpu_pt2.cpp"
        write(filename_pt2, template_pt2.render(is_forward=False, **kwargs))
        print(f"[Backward Split] [{optimizer}]: {filename_pt2}")
...
def forward_split() -> None:
    ...
    template_pt2 = env.get_template(
        "embedding_wrapper_split_host_pt2_template.cpp",
    )
    filename_pt2 = f"gen_embedding_forward_split_wrapper_pt2.cpp"
    write(
        filename_pt2,
        template_pt2.render(
            has_gpu_support=True,
    ...
    filename_pt2 = f"gen_embedding_forward_split_wrapper_cpu_pt2.cpp"
    write(
        filename_pt2,
        template_pt2.render(
