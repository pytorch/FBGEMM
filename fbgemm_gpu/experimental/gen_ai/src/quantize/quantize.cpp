...
std::vector<at::Tensor> f8f8bf16_rowwise_grouped(
    ...
    std::optional<at::Tensor> M_values = std::nullopt,
...
#ifdef USE_ROCM
  m.def(
      "f8f8bf16_rowwise_grouped(Tensor[] XQ, Tensor[] WQ, Tensor[] x_scale, Tensor[] w_scale, Tensor[](a!)? output=None, Tensor? M_values=None, str? kernel_name=None) -> Tensor[]");
