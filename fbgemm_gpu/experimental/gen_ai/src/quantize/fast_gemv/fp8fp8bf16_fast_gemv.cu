...
void fp8fp8FastGemvKernel(
  ...
  // matrix block_size is represented as (block_dim.x, block_dim.y).
  // grid_dim is accordingly calculated number of threadblocks needed to cover
  ...
  if (block_dim.x == 128) {
    gemv_quantized_fp8_fp8<TILE_M, TILE_N, 128>
        <<<grid_dim, block_dim, 0, stream>>>(
            mat, vec, res, k, m, n, scale, num_per_thread);
...
bool fastGemvLauncher(
    ...
    const unsigned int n,
    float const* scale) {
  return fastGemvTemplateCaller<1, 2>(mat, vec, res, k, m, n, scale);
}
