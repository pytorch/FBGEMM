...
class TuningCache final {
 public:
  ...
  explicit TuningCache(const std::string& kernelName)
    ...
    LOG(INFO) << fmt::format("Using cache file at {}", cacheFilename_);
 ...
 private:
  ...
  Kernel getKernel(
    ...
    TORCH_CHECK(
        it != kernels.end(),
        fmt::format(
            "Failed to find kernel keyed by {}. Consider deleting your fbgemm cache (~/.fbgemm).",
            kernel_key));
    return it->second;
  }
  ...
  std::string getCacheDirectory() {
    ...
    return fmt::format("{}/{}", getenv("HOME"), FBGEMM_CACHE_DIR);
  }

  std::string getCacheFilename(const std::string& kernel_name) {
    return fmt::format("{}/{}.txt", getCacheDirectory(), kernel_name);
  }

  std::string getDetailedFilename(const std::string& kernel_name) {
    return fmt::format("{}/{}_detailed.txt", getCacheDirectory(), kernel_name);
  }
    ...
    if (!cacheDirExists()) {
      ...
      if (!cacheDirExists()) {
        LOG(FATAL) << fmt::format(
            "FBGEMM cache directory creation at {} failed: {}.",
            cacheDirectory_,
            error);
      }
    }
  ...
  void saveCache() {
    ...
    if (file.is_open()) {
      for (const auto& pair : cache_) {
        file << fmt::format("{}={}", pair.first, pair.second) << std::endl;
      }
      file.close();
    }
      ...
      if (detailed_file.is_open()) {
        for (auto& [cache_key, kernels] : detailedTuningInfo_) {
          ...
          for (const auto& [kernel_name, time_ms] : kernels) {
            detailed_file << fmt::format(
                                 "{},{},{}", cache_key, kernel_name, time_ms)
                          << std::endl;
          }
        }
