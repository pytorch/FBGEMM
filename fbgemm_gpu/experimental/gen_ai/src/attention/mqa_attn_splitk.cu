...
// Use fewer warps for mqa_attn_splitk_wmma_kernel
...
__global__ void mqa_attn_splitk_reduce_wmma_kernel(
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> mqa_attn_splitk_cuda(
  ...
  TORCH_CHECK(
      dprops->major >= 8,
      "Too old compute capability major version to run mqa_attn_splitk_wmma ",
      dprops->major);
  ...
  if (cache_K.dtype() == at::kBFloat16) {
    ...
    TORCH_CHECK(
      num_int4_kv_groups == 1 ||
      num_int4_kv_groups == 4,
      "Invalid num_int4_kv_groups ", num_int4_kv_groups
    );
  ...
  // This is called ldc inside mqa_attn_splitk_wmma_kernel kernel
...
#define CALL_MQA_ATTN_SPLITK_WMMA(CACHE_TYPE, NUM_GROUPS, KV_LOAD_T)        \
  const auto mqa_fn =                                                       \
      mqa_attn_splitk_wmma_kernel<CACHE_TYPE, NUM_GROUPS, KV_LOAD_T>;       \
  if (smem > SMEM_ADJUST_THRESHOLD) {                                       \
    set_gpu_max_dynamic_shared_memory(mqa_fn, smem, XQ.get_device());       \
  }                                                                         \
  mqa_fn<<<blocks, threads, smem, at::cuda::getCurrentCUDAStream()>>>(      \
  ...
  if (cache_K.dtype() == at::kBFloat16) {
    CALL_MQA_ATTN_SPLITK_WMMA(at::BFloat16, 1, uint32_t);
  } else {
    if (num_int4_kv_groups == 1) {
      CALL_MQA_ATTN_SPLITK_WMMA(uint8_t, 1, uint32_t);
    } else {
      CALL_MQA_ATTN_SPLITK_WMMA(uint8_t, 4, uint2);
    }
  }

#undef CALL_MQA_ATTN_SPLITK_WMMA

  mqa_attn_splitk_reduce_wmma_kernel<<<
      dim3(B, H),
