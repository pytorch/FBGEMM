...
__global__ void efficient_attention_forward_decoder_ck_kernel(
  ...
  const auto wavefront_idx = threadIdx.y;
  const auto threads_per_wavefront = blockDim.x;
  const auto wavefronts_per_block = blockDim.y;
