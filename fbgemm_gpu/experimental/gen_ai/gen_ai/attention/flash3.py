...
try:
    ...
    from .utils._cpp_lib import _build_metadata
...
def _heuristic_kvsplit(
    ...
    if atten_bias.q_seqinfo.min_seqlen != atten_bias.q_seqinfo.max_seqlen:
        return False

    # filter out prefill case
    # pyre-ignore Undefined attribute [16]
    if atten_bias.q_seqinfo.max_seqlen == atten_bias.k_seqinfo.max_seqlen:
        return False
...
class FwOp(AttentionFwOpBase):
    ...
    def apply(
        ...
        unpack_func = lambda x: x.unpack() if isinstance(x, ScaledTensor) else (x, None)  # noqa E731
