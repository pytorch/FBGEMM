...
from . import attn_bias as fmha_attn_bias, triton_splitk
...
def create_attn_bias(  # noqa C901
    ...
    if bias_type is torch.Tensor:
        if fmt == "BMK":
            ...
            num_heads = 1
        if op is not None and issubclass(op, triton_splitk.FwOp):
            attn_bias = (
                torch.randn(
        ...
        if fmt == "BMK":
            attn_bias = attn_bias[:, 0]
        return attn_bias
    if bias_type is fmha_attn_bias.LowerTriangularMask:
        return bias_type()
    if bias_type is fmha_attn_bias.LowerTriangularFromBottomRightMask:
        return bias_type()
    if bias_type is fmha_attn_bias.LowerTriangularFromBottomRightLocalAttentionMask:
        return bias_type(window_size)
    if bias_type is fmha_attn_bias.LowerTriangularMaskWithTensorBias:
        attn_bias = _create_aligned_bias(
            batch_size,
            num_heads_groups,
            num_heads,
        ...
        if requires_grad:
            attn_bias.requires_grad_(True)
        return fmha_attn_bias.LowerTriangularMaskWithTensorBias(attn_bias)
    if bias_type in [
        fmha_attn_bias.BlockDiagonalMask,
        fmha_attn_bias.BlockDiagonalCausalMask,
        fmha_attn_bias.BlockDiagonalCausalFromBottomRightMask,
        fmha_attn_bias.BlockDiagonalCausalLocalAttentionMask,
        fmha_attn_bias.BlockDiagonalCausalLocalAttentionFromBottomRightMask,
    ]:
        ...
        if bias_type in {
            ...
            max_q_minus_k = 0
        elif bias_type == fmha_attn_bias.BlockDiagonalCausalLocalAttentionMask:
            assert window_size is not None
            max_q_minus_k = window_size - 1

        block_diag = fmha_attn_bias.BlockDiagonalMask.from_seqlens(
            *_rand_seqlens(
                r,
                batch_size,
                q_len,
        ...
        if bias_type is fmha_attn_bias.BlockDiagonalCausalMask:
            block_diag = block_diag.make_causal()
        if bias_type in {
            fmha_attn_bias.BlockDiagonalCausalLocalAttentionMask,
            fmha_attn_bias.BlockDiagonalCausalLocalAttentionFromBottomRightMask,
        }:
            block_diag = fmha_attn_bias.BlockDiagonalMask(
                q_seqinfo=block_diag.q_seqinfo,
                k_seqinfo=block_diag.k_seqinfo,
                _batch_sizes=block_diag._batch_sizes,
            )
            assert window_size is not None
            if bias_type is fmha_attn_bias.BlockDiagonalCausalLocalAttentionMask:
                block_diag = block_diag.make_local_attention(window_size)
            else:
                block_diag = block_diag.make_local_attention_from_bottomright(
                    window_size
                )
        if bias_type is fmha_attn_bias.BlockDiagonalCausalFromBottomRightMask:
            block_diag = block_diag.make_causal_from_bottomright()
        return block_diag
    if bias_type in [
        fmha_attn_bias.BlockDiagonalPaddedKeysMask,
        fmha_attn_bias.BlockDiagonalLocalAttentionPaddedKeysMask,
        fmha_attn_bias.BlockDiagonalCausalLocalAttentionPaddedKeysMask,
        fmha_attn_bias.BlockDiagonalCausalWithOffsetPaddedKeysMask,
        fmha_attn_bias.PagedBlockDiagonalPaddedKeysMask,
        fmha_attn_bias.PagedBlockDiagonalCausalWithOffsetPaddedKeysMask,
    ]:
        assert fmt in ["BMHK", "BMGHK"]
        q, k = _rand_seqlens_padded_k(r, batch_size, q_len, kv_len)
        block_diag_type = (
            bias_type._UNPAGED_TYPE
            if issubclass(bias_type, fmha_attn_bias.PagedBlockDiagonalPaddedKeysMask)
            else bias_type
        )
        if bias_type is fmha_attn_bias.BlockDiagonalCausalLocalAttentionPaddedKeysMask:
        ...
        elif bias_type is fmha_attn_bias.BlockDiagonalLocalAttentionPaddedKeysMask:
            g_block_diag = block_diag_type.from_seqlens_local(
                q_seqlen=q,
                kv_padding=kv_len,
                kv_seqlen=k,
        ...
        return g_block_diag
    if bias_type in [
        fmha_attn_bias.BlockDiagonalCausalWithOffsetGappyKeysMask,
        fmha_attn_bias.BlockDiagonalGappyKeysMask,
    ]:
        assert fmt in ["BMHK", "BMGHK"]
        max_q_minus_k = (
            None if bias_type is fmha_attn_bias.BlockDiagonalGappyKeysMask else 0
        )
        ...
        return fmha_attn_bias.BlockDiagonalGappyKeysMask.from_seqlens(
            q_seqlen=q,
            kv_seqstarts=starts,
            kv_seqlen=k,
        )
    if issubclass(bias_type, fmha_attn_bias.PagedBlockDiagonalGappyKeysMask):
        assert fmt in ["BMHK", "BMGHK"]
        assert page_size is not None
        pages_per_row = (kv_len + page_size - 1) // page_size
        total_queries = q_len * batch_size
        if issubclass(
            bias_type, fmha_attn_bias.PagedBlockDiagonalCausalWithOffsetGappyKeysMask
        ):
            q, k = _rand_seqlens_padded_k(r, batch_size, q_len, kv_len)
        else:
    ...
    if bias_type == fmha_attn_bias.LocalAttentionFromBottomRightMask:
        return bias_type(
...
def ref_attention(q, k, v, attn_bias=None, drop_mask=None, p=0.0, scale=None):
    if q.ndim == 5:
        ...
        def attn_bias_group(group: int):
            ...
            if isinstance(attn_bias, fmha_attn_bias.LowerTriangularMaskWithTensorBias):
                return fmha_attn_bias.LowerTriangularMaskWithTensorBias(
    ...
    if attn_bias is not None:
        if isinstance(attn_bias, fmha_attn_bias.AttentionBias):
...
def pack_kv_cache(
    ...
    seqstarts = (
        ...
        .to(device=torch.accelerator.current_accelerator())
        .unsqueeze(1)
    ) // BLOCK_N
