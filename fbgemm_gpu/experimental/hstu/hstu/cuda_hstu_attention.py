...
def quantize_for_two_directions(
    x: torch.Tensor,
    seq_offsets: torch.Tensor,
    fp8_type: torch.dtype = torch.float8_e4m3fn,
...
def quantize_for_block_scale(
    ...
    x_quantized_list: list[torch.Tensor] = []
    x_descale_list: list[torch.Tensor] = []

    with torch.no_grad():
        for i in range(B):
            actual_len = int(seq_offsets[i + 1] - seq_offsets[i])
...
def get_bm_and_bn_block_size_fwd(rab: torch.Tensor | None, dim: int) -> tuple[int, int]:
    """
...
def get_bm_and_bn_block_size_bwd() -> tuple[int, int]:
    """
    ...
    def forward(  # pyre-ignore[14]
        ...
        if major_version == 8:
            ...
            if quant_mode == 0:
                ...
                q, q_descale, _, _, _ = quantize_for_two_directions(
                    q, cu_seqlens_q, fp8_type=torch.float8_e4m3fn
                )
                k, k_descale, _, _, _ = quantize_for_two_directions(
                    k, cu_seqlens_k, fp8_type=torch.float8_e4m3fn
                )
                v, v_descale, vt, vt_descale, cu_seqlens_vt_descale = (
        ...
        *args: Any,
        ...
        if ctx.major_version == 8:
            ...
            if quant_mode == 0:
                ...
                dout, do_descale, _ = quantize_for_block_scale(
                ...
                dout, do_descale = quantize_for_head_batch_tensor(
...
def hstu_attn_varlen_func(
    ...
    if num_targets is None and target_group_size is not None and target_group_size < 1:
        raise ValueError(
