...
def generate_kernels_ampere(install_dir: str):
    ...
    RAB_DRAB = [""] + (
        (["_rab_drab", "_rab"])
        if not DISABLE_DRAB
        else ["_rab"] if not DISABLE_RAB else []
    )
    MASK = [""]
...
template void run_hstu_fwd_8x<{}, {}, {}, {}, {}, {}, {}, {}, {}, {}>
    ...
    for hdim, dtype, rab, mask, arch_sm in itertools.product(
        ...
        file_name = (
            f"{install_dir}/hstu_fwd_hdim{hdim}_{dtype}{rab}{mask}_fn{ARBITRARY_NFUNC}_sm{arch_sm}.cu"
            if "arbitrary" in mask
            else f"{install_dir}/hstu_fwd_hdim{hdim}_{dtype}{rab}{mask}_sm{arch_sm}.cu"
        )
...
template void run_hstu_bwd_80<{}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}>
    ...
    for hdim, dtype, rab_drab, mask, bwd_deterministic, arch_sm in itertools.product(
        ...
        file_name = (
            f"{install_dir}/hstu_bwd_hdim{hdim}_{dtype}{rab_drab}{mask}_fn{ARBITRARY_NFUNC}_{bwd_deterministic}_sm{arch_sm}.cu"
            if "arbitrary" in mask
            else f"{install_dir}/hstu_bwd_hdim{hdim}_{dtype}{rab_drab}{mask}_{bwd_deterministic}_sm{arch_sm}.cu"
        )
        if not os.path.exists(file_name):
            with open(file_name, "w") as f:
                f.write(
...
template void run_hstu_fwd_<90, {}, {}, {}, {}, {}, {}, {}, {}, {}>
    ...
    for hdim, dtype, rab, mask in itertools.product(
        ...
        file_name = (
            f"{install_dir}/hstu_fwd_hdim{hdim}_{dtype}{rab}{mask}_fn{ARBITRARY_NFUNC}_sm90.cu"
            if "arbitrary" in mask
            else f"{install_dir}/hstu_fwd_hdim{hdim}_{dtype}{rab}{mask}_sm90.cu"
        )
        if not os.path.exists(file_name):
            with open(file_name, "w") as f:
                f.write(
    ...
    if not DISABLE_FP8:
        for hdim, rab, mask in itertools.product(HEAD_DIMENSIONS, RAB, MASK):
            if hdim == 32:
                continue
            file_name = (
                f"{install_dir}/hstu_fwd_hdim{hdim}_e4m3{rab}{mask}_fn{ARBITRARY_NFUNC}_sm90.cu"
                if "arbitrary" in mask
                else f"{install_dir}/hstu_fwd_hdim{hdim}_e4m3{rab}{mask}_sm90.cu"
            )
            if not os.path.exists(file_name):
                with open(file_name, "w") as f:
                    f.write(
...
template void run_hstu_bwd_<90, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}>
    ...
    for hdim, dtype, rab_drab, mask in itertools.product(
        ...
        file_name = (
            f"{install_dir}/hstu_bwd_hdim{hdim}_{dtype}{rab_drab}{mask}_fn{ARBITRARY_NFUNC}_sm90.cu"
            if "arbitrary" in mask
            else f"{install_dir}/hstu_bwd_hdim{hdim}_{dtype}{rab_drab}{mask}_sm90.cu"
        )
        if not os.path.exists(file_name):
            with open(file_name, "w") as f:
                f.write(
    ...
    if not DISABLE_FP8 and not USE_E5M2_BWD:
        for hdim, rab_drab, mask in itertools.product(HEAD_DIMENSIONS, RAB_DRAB, MASK):
            if hdim == 32:
                continue
            file_name = (
                f"{install_dir}/hstu_bwd_hdim{hdim}_e4m3{rab_drab}{mask}_fn{ARBITRARY_NFUNC}_sm90.cu"
                if "arbitrary" in mask
                else f"{install_dir}/hstu_bwd_hdim{hdim}_e4m3{rab_drab}{mask}_sm90.cu"
            )
            if not os.path.exists(file_name):
                with open(file_name, "w") as f:
                    f.write(
    ...
    if not DISABLE_FP8 and USE_E5M2_BWD:
        for hdim, rab_drab, mask in itertools.product(HEAD_DIMENSIONS, RAB_DRAB, MASK):
            if hdim == 32:
                continue
            file_name = (
                f"{install_dir}/hstu_bwd_hdim{hdim}_e5m2{rab_drab}{mask}_fn{ARBITRARY_NFUNC}_sm90.cu"
                if "arbitrary" in mask
                else f"{install_dir}/hstu_bwd_hdim{hdim}_e5m2{rab_drab}{mask}_sm90.cu"
            )
            if not os.path.exists(file_name):
                with open(file_name, "w") as f:
                    f.write(
