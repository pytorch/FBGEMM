...
@click.option("--user-tables", default=300, type=int)
...
def main(num_ads, embedding_dimension, ads_tables, user_tables, iters, p2p_bw, dst_device) -> None:
    ...
    pooled_user_embeddings = [
        torch.randn(
            num_users, user_d, dtype=torch.float16, device=torch.cuda.current_device()
        ).half()
