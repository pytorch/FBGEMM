...
@click.option(
    ...
    type=int,
    default=100,
    help="The number of iterations to run the benchmark for.",
)
@click.option(
    "--warmup-runs",
    type=int,
    default=0,
    help="The number of warm-up runs to do before benchmarking.",
)
@click.option(
    "--weights-precision",
    type=SparseType,
    default=SparseType.FP32,
    help="The data type of weights.",
)
@click.option(
    "--cache-precision",
    type=SparseType,
    default=SparseType.FP32,
    help="The data type of caches. If not set, this is automatically inferred.",
)
@click.option(
    "--stoc",
    is_flag=True,
    default=False,
    help=" The flag to enable stochastic rounding.",
)
@click.option(
    "--reuse",
    default=0.0,
    type=float,
    help="The ratio of reusing indices (0.0 <= reuse <= 1.0).",
)
@click.option(
    "--pooling",
...
@click.option(
    ...
    help=str(
        "The bound-check mode to use: "
        + ", ".join([f"{mode.value} : {mode.name}" for mode in BoundsCheckMode])
    ),
)
@click.option(
    "--output-dtype",
...
def vbe(
    ...
    for t, (E, B, L, sigma_L, alpha) in enumerate(zip(Es, Bs, Ls, sigma_Ls, alphas)):
        ...
        for i, req in enumerate(local_requests):
            indices, offsets, _ = req.unpack_3()
            all_requests["indices"][i].append(indices)
            if t > 0:
                offsets = offsets[1:]  # remove the first element
                offsets += all_requests["offsets"][i][t - 1][-1]
