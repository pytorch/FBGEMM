...
TEST(StoreValueUtils, BasicFunctionality) {
  ...
  StoreValueUtils::update_timestamp(block, now);
  ...
  ASSERT_EQ(
      ...
      sizeof(StoreValueUtils::MetaHeader));

  // Test timestamp update
  now = facebook::WallClockUtil::NowInUsecFast();
  StoreValueUtils::update_timestamp<float>(block, now);
...
TEST(StoreValueUtils, MultiDimensionTest) {
  ...
  for (int dim : test_dims) {
    ...
    const size_t expected_size =
        sizeof(StoreValueUtils::MetaHeader) + dim * sizeof(float);
...
TEST(StoreValueUtils, TimestampPrecision) {
  ...
  for (int i = 0; i < test_iterations; ++i) {
    ...
    EXPECT_GE(
        curr_ts, prev_ts); // Timestamps should be monotonically increasing
    prev_ts = curr_ts;
  }
}
