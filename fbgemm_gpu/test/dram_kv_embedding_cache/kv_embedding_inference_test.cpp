...
TEST_F(KVEmbeddingInferenceTest, ConcurrentBatchOperations) {
  LOG(INFO) << "=== ConcurrentBatchOperations Test ===";

  constexpr int NUM_EMBEDDINGS = 10000;
  constexpr int BATCH_SIZE = 64;
  constexpr int NUM_READERS = 32;
  constexpr int NUM_WRITERS = 32;
  constexpr int BATCHES_PER_THREAD = 500;

  LOG(INFO) << "Pre-populating " << NUM_EMBEDDINGS << " embeddings";
  bulkInsert(0, NUM_EMBEDDINGS);
  ...
  LOG(INFO) << "Starting batch operations (batch_size=" << BATCH_SIZE
            << ", batches/thread=" << BATCHES_PER_THREAD << ")";

  // Batch reader threads
  for (int t = 0; t < NUM_READERS; ++t) {
    threads.emplace_back([this, t, &batch_reads, &errors]() {
      std::mt19937 rng(t + 3000);
      std::uniform_int_distribution<int64_t> dist(
          0, NUM_EMBEDDINGS - BATCH_SIZE);

      for (int b = 0; b < BATCHES_PER_THREAD; ++b) {
        try {
          int64_t start_id = dist(rng);
          std::vector<int64_t> batch_indices(BATCH_SIZE);
          for (int i = 0; i < BATCH_SIZE; ++i) {
            batch_indices[i] = start_id + i;
          }
          ...
          auto indices = at::from_blob(
                             ...
                             .clone();
          auto weights = at::zeros({BATCH_SIZE, EMBEDDING_DIM}, at::kFloat);
          auto count = at::tensor({BATCH_SIZE}, at::kInt);

          folly::coro::blockingWait(
              backend_->get_kv_db_async(indices, weights, count));
          batch_reads.fetch_add(1, std::memory_order_relaxed);

          auto* data = weights.const_data_ptr<float>();
          for (int64_t i = 0; i < BATCH_SIZE * EMBEDDING_DIM; ++i) {
  ...
  for (int t = 0; t < NUM_WRITERS; ++t) {
    threads.emplace_back([this, t, &batch_writes, &errors]() {
      ...
      for (int b = 0; b < BATCHES_PER_THREAD; ++b) {
        try {
          int64_t start_id = dist(rng);
          std::vector<int64_t> batch_indices(BATCH_SIZE);
          std::vector<float> batch_weights(BATCH_SIZE * EMBEDDING_DIM);

          for (int i = 0; i < BATCH_SIZE; ++i) {
            batch_indices[i] = start_id + i;
            auto embedding = generateEmbedding(start_id + i + b * 100000);
            std::copy(
                embedding.begin(),
                embedding.end(),
                batch_weights.begin() + i * EMBEDDING_DIM);
          }
          ...
          folly::coro::blockingWait(backend_->inference_set_kv_db_async(
              indices, weights, count, 4000 + t * 100 + b));
          batch_writes.fetch_add(1, std::memory_order_relaxed);
        } catch (...) {
          errors.fetch_add(1, std::memory_order_relaxed);
        }
      }
  ...
  int total_batches = batch_reads.load() + batch_writes.load();
  int total_items = total_batches * BATCH_SIZE;
  double items_per_sec =
      static_cast<double>(total_items) / duration_ms * 1000.0;

  LOG(INFO) << "Completed in " << duration_ms << "ms";
  LOG(INFO) << "Batch reads: " << batch_reads.load()
            << ", Batch writes: " << batch_writes.load();
  LOG(INFO) << "Total items: " << total_items << " (" << items_per_sec
            << " items/sec)";
  LOG(INFO) << "Errors: " << errors.load();

  ASSERT_EQ(errors.load(), 0);
  ASSERT_EQ(batch_reads.load(), NUM_READERS * BATCHES_PER_THREAD);
  ASSERT_EQ(batch_writes.load(), NUM_WRITERS * BATCHES_PER_THREAD);
}
