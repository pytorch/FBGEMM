...
class SSDSplitTableBatchedEmbeddingsTest(SSDSplitTableBatchedEmbeddingsTestCommon):
    ...
    def test_direct_write_with_async_backend_fetch_synchronization(
        ...
        output_1 = self.execute_ssd_forward_(
            emb,
            emb_ref,
            indices_list_1,
            per_sample_weights_list_1,
        ...
        # Store scratch pad queue length before direct_write
        scratch_pads_before = len(emb.ssd_scratch_pads) if prefetch_pipeline else 0
        ...
        # Assert: Verify synchronization happened correctly
        # The scratch pad queues should be properly maintained
        scratch_pads_after = len(emb.ssd_scratch_pads) if prefetch_pipeline else 0

        # After direct_write, one scratch pad should be popped
        if prefetch_pipeline and scratch_pads_before > 0:
            self.assertEqual(
                scratch_pads_after,
                scratch_pads_before - 1,
                "direct_write should pop one scratch pad from queue",
            )
    ...
    def test_direct_write_clears_location_update_queue(
        ...
        output = self.execute_ssd_forward_(
            emb,
            emb_ref,
            indices_list,
            per_sample_weights_list,
