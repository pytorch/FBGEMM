...
inline __device__ void embedding_inplace_update_kernel_impl(
  ...
  // kernel_1 repeats this check before its own weights_placements read; this
  // one covers kernel_2, which is passed the placement rather than indexing it.
...
__launch_bounds__(kMaxThreads) __global__
  ...
  for (int64_t i = blockIdx.x * blockDim.y + threadIdx.y; i < N;
    ...
    const int32_t table_idx = update_table_idx[i];
    // Must be checked here, before weights_placements[table_idx]: this kernel
    // reads per-table metadata itself, so the identical assert inside
    // embedding_inplace_update_kernel_impl would fire only after an
    // out-of-range table_idx had already read out of bounds.
    CUDA_KERNEL_ASSERT(
        table_idx >= 0 && table_idx < weights_placements.size(0) &&
        "embedding_inplace_update: table_idx out of range");
