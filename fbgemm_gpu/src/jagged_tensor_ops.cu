...
bool jagged_dense_dense_elementwise_jagged_output_matches_opt(
  ...
  matches &= (reinterpret_cast<uint64_t>(x_values.data_ptr()) % 8 == 0);
  matches &= (reinterpret_cast<uint64_t>(output_values.data_ptr()) % 8 == 0);
  matches &= (reinterpret_cast<uint64_t>(y_0_reshaped.data_ptr()) % 8 == 0);
  matches &= (reinterpret_cast<uint64_t>(y_1_reshaped.data_ptr()) % 8 == 0);
    ...
    AT_DISPATCH_INDEX_TYPES(
        ...
        [=] {
          ...
          jagged_dense_dense_elementwise_jagged_output_opt_gather_kernel_<
              ...
              nnz,
              E,
              [_f = f] __device__(__half x, __half y0, __half y1) -> __half {
                return _f(x, y0, y1);
              });
          C10_CUDA_KERNEL_LAUNCH_CHECK();
        }); // AT_DISPATCH
