      ...
      [&] {
        AT_DISPATCH_INDEX_TYPES(
            ...
            [&] {
              ...
              const uint32_t segment_blocks_x =
              ...
              const uint32_t blocks_y = std::clamp<uint32_t>(
                  (kBlocksPerSM * sm_count + segment_blocks_x - 1) /
                      segment_blocks_x,
                  1,
                  kMaxSegmentFanout);
              ...
              // so capping is correctness-preserving. Fold blocks_y into the
              // per-block thread count so the gridDim.y fan-out is bounded by
              // the same cap instead of multiplying the total threads after it.
              // See: https://github.com/ROCm/hip/issues/2253
              const uint32_t blocks_x = utils::cuda::cap_grid_dim_x(
                  segment_blocks_x,
                  static_cast<int64_t>(threads.x) * threads.y * blocks_y,
                  at::cuda::getCurrentCUDAStream());
