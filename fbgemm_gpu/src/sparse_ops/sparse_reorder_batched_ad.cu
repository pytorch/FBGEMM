...
template <typename Dtype, typename index_t = int32_t>
__global__
__launch_bounds__(fbgemm_gpu::kMaxThreads) void reorder_batched_ad_indices_kernel_dtypeLong(
    // reorder indices from (ragged) [B  x T x #num_ads_b x length_{b, t, a})]
    // to [T][B][#num_ads_b][length_{b, t, a}], i.e. [sum(length_{b, t, a})],
    // laid out as [T][B][A][L] (if all lengths were equal).
  ...
  if (broadcast_indices) {
    ...
    if (num_elements <= 64) {
      ...
      auto dst =
          (long2*)(reordered_cat_ad_indices.data() + output_segment_start);
      auto src = (long2*)(cat_ad_indices.data() + input_segment_start);
      ...
      auto dst =
          (long4*)(reordered_cat_ad_indices.data() + output_segment_start);
      auto src = (long4*)(cat_ad_indices.data() + input_segment_start);
...
template <typename Dtype, typename index_t = int32_t>
__global__
__launch_bounds__(fbgemm_gpu::kMaxThreads) void reorder_batched_ad_indices_kernel_dtypeFloat(
    // reorder indices from (ragged) [B  x T x #num_ads_b x length_{b, t, a})]
    // to [T][B][#num_ads_b][length_{b, t, a}], i.e. [sum(length_{b, t, a})],
    // laid out as [T][B][A][L] (if all lengths were equal).
  ...
  if (broadcast_indices) {
    ...
    if (num_elements <= 64) {
      ...
      auto dst =
          (float2*)(reordered_cat_ad_indices.data() + output_segment_start);
      auto src = (float2*)(cat_ad_indices.data() + input_segment_start);
      for (auto i = threadIdx.x; i < num_elements / 2; i += blockDim.x) {
        dst[i] = src[i];
      }
      if ((num_elements % 2) && threadIdx.x == 31) {
      ...
      auto dst =
          (float4*)(reordered_cat_ad_indices.data() + output_segment_start);
      auto src = (float4*)(cat_ad_indices.data() + input_segment_start);
      for (auto i = threadIdx.x; i < num_elements / 4; i += blockDim.x) {
        dst[i] = src[i];
      }
      int remainder = num_elements % 4;
