...
void _segment_sum_csr_cpu_kernel(
    ...
    const U* const csr_seg_data,
    const T* const values_data,
    T* const output_data) {
  for (const auto i : c10::irange(num_segments)) {
    const int64_t seg_start = csr_seg_data[i] * batch_size;
    const int64_t seg_end = csr_seg_data[i + 1] * batch_size;
    T v = 0;
      ...
      values.scalar_type(), "_segment_sum_csr_cpu_1", [&] {
        ...
        AT_DISPATCH_INDEX_TYPES(
            csr_seg.scalar_type(), "_segment_sum_csr_cpu_2", [&] {
              ...
              _segment_sum_csr_cpu_kernel<value_t, csr_seg_t>(
                  csr_seg.numel() - 1,
                  batch_size,
