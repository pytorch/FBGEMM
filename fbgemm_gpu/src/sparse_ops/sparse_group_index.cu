...
__launch_bounds__(kMaxThreads) void group_index_select_or_add_2d_kernel(
  ...
  // Sentinel for "accumulator empty". index_t::max() is safe as a sentinel:
  // valid indices are row offsets bounded by the embedding table height, which
  // is far below the max of index_t (int32/int64), so it never collides with a
  // real index.
  ...
  if constexpr (
      !USE_INDEX_SELECT && !USE_VAR_COLS && !USE_CACHE && !USE_SORTED_INDICES &&
      !USE_CONTIGUOUS_WARPS && USE_PACKED_ROWS) {
...
#pragma unroll
          for (int i = 0; i < UNROLL_FACTOR && col_offset + i < num_cols; i++) {
            gpuAtomicAddNoReturn(
                &output[static_cast<int64_t>(idx) * num_cols + i],
                input[current_row * num_cols + i]);
          }
        }
      } else {
        // Wide-column fallback: >= 1 warp per row.
        // int64 so the row * num_cols address arithmetic below cannot overflow.
...
#pragma unroll
  ...
  auto flush_cache_accumulator = [&](scalar_t* target_output,
    ...
    if constexpr (!USE_INDEX_SELECT && USE_CACHE) {
      if (target_output && cached_idx != kInvalidIdx) {
        gpuAtomicAddNoReturn(
            &target_output[static_cast<int64_t>(cached_idx) * target_num_cols],
            storage);
  ...
  for (int64_t warp_id = start_warp_id; warp_id < warp_end;
       warp_id += warp_stride) {
    ...
    if constexpr (USE_PACKED_ROWS) {
      if (use_small_dim_path) {
        ...
        // [COLS_PER_WARP % num_cols != 0] and we also need to confirm that we
        // are within num_work_rows
        if (local_row < rows_per_warp && current_row < num_work_rows) {
    ...
    if (!handled_small_dim_path) {
      ...
      int32_t col_tile = 0;
      if constexpr (USE_CONTIGUOUS_WARPS) {
        // Contiguous warp traversal: iterate rows sequentially while column
        // tiles remain strided so each warp processes a different tile for
        // successive rows.
        row_in_member = member_warp_id % num_work_rows;
        col_tile = member_warp_id / num_work_rows;
      } else {
        // Original strided mapping: each warp walks tiles first, distributing
        // rows round-robin.
