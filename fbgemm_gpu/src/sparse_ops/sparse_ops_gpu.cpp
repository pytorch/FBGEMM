...
static torch::autograd::variable_list group_index_select_dim0_forward_impl_gpu(
  ...
  const bool enable_cache_and_contig_for_bwd = wide_enough_for_cache &&
  ...
  const bool use_sorted_indices_for_bwd = enable_cache_and_contig_for_bwd &&
...
static torch::autograd::variable_list group_index_select_dim0_backward_impl_gpu(
  ...
  [[maybe_unused]] const bool enable_cache_and_contig = saved_data_ptr[7];
  ...
  for (int i = 0; i < group_size; i++) {
    output_group[i] = output_group[i].reshape(
        c10::IntArrayRef(
  ...
  // All groups share the same index dtype and size (enforced by forward
  // checks).
  ...
  if (use_sorted_indices) {
    ...
    sort_original_positions =
        at::arange(sort_num_items, first_indices.options().dtype(at::kLong));
    // Single contiguous allocation for all groups' sort outputs.
    // record_stream called twice total instead of 2*group_size.
    all_sorted_indices =
        at::empty({group_size * sort_num_items}, first_indices.options());
    ...
    // These scratch tensors are also consumed by async rocprim launches on
    // `stream`; record them too so the caching allocator can't recycle them
    // before the sort completes if the current stream differs from the alloc
    // stream.
    sort_temp_storage.record_stream(stream);
    sort_original_positions.record_stream(stream);
  }
#endif
  for (const auto i : c10::irange(group_size)) {
    const auto& indices = indices_group[i];
  ...
  if (use_sorted_indices) {
    ...
    for (int64_t i = 0; i < group_size; ++i) {
      sorted_indices_ptrs[i] = reinterpret_cast<int64_t>(
          sorted_base + i * sort_num_items * idx_elem_bytes);
      reverse_indices_ptrs[i] = reinterpret_cast<int64_t>(
          reverse_base + i * sort_num_items * sizeof(int64_t));
    }
    ...
    if (use_segmented_sort) {
      ...
      // Values are [0..N-1] tiled per segment. Build in one allocation via
      // repeat (no expand()+contiguous() intermediate copy).
      const auto all_values_in = sort_original_positions.repeat(group_size);
      // Offsets [0, N, 2N, ..., group_size*N] in one strided arange (avoids the
      // separate arange + scalar-multiply temporary).
      const auto all_options = first_indices.options().dtype(at::kLong);
      const auto segment_offsets = at::arange(
          /*start=*/0,
          /*end=*/static_cast<int64_t>(group_size + 1) * sort_num_items,
          /*step=*/sort_num_items,
          all_options);
