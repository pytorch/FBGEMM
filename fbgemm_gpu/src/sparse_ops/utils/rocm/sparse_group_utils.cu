...
DLL_PUBLIC size_t get_sort_temp_storage_bytes(
  ...
  AT_DISPATCH_INDEX_TYPES(scalar_type, "get_sort_temp_storage_bytes", [&] {
    AT_CUDA_CHECK(
        rocprim::radix_sort_pairs<sort_config>(
            nullptr,
            temp_storage_bytes,
...
DLL_PUBLIC size_t get_segmented_sort_temp_storage_bytes(
  ...
  const size_t total_items =
      num_items_per_segment * static_cast<size_t>(num_groups);
  AT_DISPATCH_INDEX_TYPES(
      scalar_type, "get_segmented_sort_temp_storage_bytes", [&] {
        // segmented_radix_sort_pairs requires segmented_radix_sort_config, not
        // radix_sort_config — use default config (radix sort, no merge
        // fallback).
        AT_CUDA_CHECK(
...
DLL_PUBLIC void sort_indices_segmented_rocprim(
  ...
  const size_t total_items =
      num_items_per_segment * static_cast<size_t>(num_groups);
  // segment_offsets is [0, N, 2N, ..., K*N]: begin[i] = ptr[i], end[i] =
  // ptr[i+1]
