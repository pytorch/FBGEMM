...
TEST(KvDbTableBatchedEmbeddingsTest, TestTensorStream) {
  ...
  auto mockService = std::make_shared<MockTrainingParameterServerService>();
  auto mockServer =
  ...
  auto& mockClientFactory =
      facebook::servicerouter::getMockSRClientFactory(false /* strict */);
  mockClientFactory.registerMockService(
      "realtime.delta.publish.esr", mockServer);
  ...
  EXPECT_CALL(*mockService, co_setEmbeddings(_)).Times(0);
  ...
  weights = at::randn(
      {ind.size(0), 8},
      at::TensorOptions().device(at::kCPU).dtype(c10::kFloat));
  EXPECT_CALL(*mockService, co_setEmbeddings(_))
