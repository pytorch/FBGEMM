...
int main(int argc, char* argv[]) {
    ...
    // cudaMemcpy(grad_get_buffer, all_to_all_grad.data_ptr<float>(), local_batch_size * nranks * total_D * sizeof(float), cudaMemcpyDeviceToDevice);

    total_B = offsets.size(0) - 1;
    std::tie(info_B_num_bits, info_B_mask) = adjust_info_B_num_bits(total_B / T, T);

    at::Tensor backward_result = nvshmem_alltoall_based_host_function(
        all_to_all_grad,
        dev_weights,
        uvm_weights,
        lxu_cache_weights,
