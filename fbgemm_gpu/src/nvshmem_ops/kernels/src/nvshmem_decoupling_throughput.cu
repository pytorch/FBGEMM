...
int main(int argc, char* argv[]) {
    ...
    int32_t if_update = 0; // put_type == 0: thread-level put;     put_type == 1: warp-level put;    put_type == 2: only_tbe + sync;     put_type >= 3: only_tbe without sync
    int32_t n_loop = 2048;
    if(argc == 3){
        if_save_result = atoi(argv[1]);
        if_update = atoi(argv[2]);
    }
    else if(argc == 4){
        if_save_result = atoi(argv[1]);
        if_update = atoi(argv[2]);
    ...
    at::Tensor backward_result = nvshmem_unsorting_backward_host_function_decoupling(
        all_to_all_grad,
        dev_weights,
        uvm_weights,
        lxu_cache_weights,
    ...
    // }
    std::cout << "indices:" << indices.size(0) << ", total_unique_indices:" << total_unique_indices << ", ratio:" << float(total_unique_indices)/float(indices.size(0)) << std::endl;
