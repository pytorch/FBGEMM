  ...
  for (const auto oidx : c10::irange(outer_dense_size)) {
    for (int joidx = 0; joidx < jagged_folded_size / jagged_innermost_size;
      ...
      if (!is_zero) {
        ...
        for (; jiidx < std::min(end - begin, static_cast<index_t>(jagged_innermost_size)); ++jiidx) {
    ...
    for (int joidx = 0; joidx < jagged_folded_size / jagged_innermost_size;
      ...
      if (!is_zero) {
        ...
        const auto end =
            x_offsets_accessors[NUM_JAGGED_DIM - 1][offset_base + 1];
        for (int jiidx = 0;
             jiidx < std::min(end - begin, static_cast<index_t>(jagged_innermost_size));
             ++jiidx) {
          int jidx = joidx * jagged_innermost_size + jiidx;
          if (NO_INNER_DENSE) {
...
void dense_vec_jagged_2d_bmm(
  ...
  for (const auto b : c10::irange(B)) {
    ...
    const auto length = std::min(row_end - row_start, static_cast<index_t>(max_L));
    if (length == 0) {
