      ...
      lengths.scalar_type(), "mask_select_jagged_1d_kernel1", [&] {
        FBGEMM_DISPATCH_ALL_TYPES(
            values.scalar_type(), "mask_select_jagged_1d_kernel2", [&] {
              ...
              const auto values_ptr = values_contiguous->const_data_ptr<scalar_t>();
              const auto lengths_ptr = lengths_contiguous->const_data_ptr<index_t>();
              const auto mask_ptr = mask_contiguous->const_data_ptr<bool>();

              auto masked_values_ptr = masked_values.mutable_data_ptr<scalar_t>();
              auto masked_lengths_ptr = masked_lengths.mutable_data_ptr<index_t>();
