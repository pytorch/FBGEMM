...
std::tuple<Tensor, Tensor, Tensor> tbe_input_combine_with_length_gpu(
  ...
  const bool need_weights = std::ranges::any_of(
      per_sample_weights, [](const auto& x) { return x.numel() > 0; });
  ...
  for (const auto i : c10::irange(num_lists)) {
    ...
    indices_addrs[i] = reinterpret_cast<uint64_t>(indices.const_data_ptr());
    lengths_addrs[i] = reinterpret_cast<uint64_t>(lengths.const_data_ptr());
    ...
    if (weights.numel() > 0) {
      ...
      per_sample_weights_addrs[i] =
          reinterpret_cast<uint64_t>(weights.const_data_ptr());
