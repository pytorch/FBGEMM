...
at::Tensor _float_to_msfp_bytes_gpu(
  ...
  at::native::gpu_kernel(
      iter, [=] GPU_LAMBDA(float in, int shared_expo) -> uint8_t {
        return quantize_msfp_float_to_uint8(
            in, shared_expo, mbits, bias, max_pos);
      });

  C10_CUDA_KERNEL_LAUNCH_CHECK();
  Tensor compact_exponents =
      at::slice(shared_exponents, 1, 0, ncols, bounding_box_size)
          .to(input.options().dtype(at::kByte));
  auto ret = at::cat({output, compact_exponents}, 1);
  return at::cat({output, compact_exponents}, 1);
}
...
at::Tensor _msfp_bytes_to_float_gpu(
  ...
  TENSOR_ON_CUDA_GPU(input);
  TENSOR_NDIM_EQUALS(input, 2);
  TORCH_CHECK(input.size(1) == (dim + int(dim / bounding_box_size)));

  at::cuda::OptionalCUDAGuard device_guard;
  device_guard.set_index(input.get_device());

  const int nrows = input.size(0);
  const int ncols = input.size(1) - int(dim / bounding_box_size);
  ...
  auto shared_exponents =
      at::unsqueeze(at::slice(input, 1, ncols), 2)
          .expand({nrows, int(dim / bounding_box_size), bounding_box_size});
  shared_exponents = shared_exponents.contiguous().view({nrows, ncols});

  auto mantissa_inputs = at::slice(input, 1, 0, ncols);
  auto iter = at::TensorIteratorConfig()
                  .check_all_same_dtype(false)
                  .add_output(output)
                  .add_input(mantissa_inputs)
                  .add_input(shared_exponents)
                  .build();

  at::native::gpu_kernel(
      iter, [=] GPU_LAMBDA(float in, int shared_expo) -> float {
        return dequantize_msfp_float_from_uint8(
            in, shared_expo, ebits, mbits, bias);
      });
