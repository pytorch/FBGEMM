...
class FeatureEvict {
 ...
 protected:
  ...
  // wait until all shards evicts are either temporarily paused or fully
  // finished, used to blocking pause() calls to guarantee no race condition
  // between the caller thread and the evict thread
  void wait_until_fully_stopped(std::unique_lock<std::mutex>& lock) {
  ...
  bool wait_until_resume(int shard_id) {
    ...
    num_waiting_evicts_++;
    if (!has_running_evict()) {
      // last paused/finished shard notify the thread waiting on pause()
      // returns
...
class CounterBasedEvict : public FeatureEvict<weight_type> {
 public:
  CounterBasedEvict(
      ...
      TestMode test_mode = TestMode::DISABLED)
      : FeatureEvict<weight_type>(kv_store, sub_table_hash_cumsum, test_mode),
...
class TimeBasedEvict : public FeatureEvict<weight_type> {
 public:
  TimeBasedEvict(
      ...
      : FeatureEvict<weight_type>(kv_store, sub_table_hash_cumsum),
        ttls_in_mins_(ttls_in_mins) {}
  ...
  switch (config->trigger_strategy_) {
    case EvictTriggerStrategy::BY_TIMESTAMP: {
      return std::make_unique<TimeBasedEvict<weight_type>>(
          kv_store, sub_table_hash_cumsum, config->ttls_in_mins_.value());
    }
