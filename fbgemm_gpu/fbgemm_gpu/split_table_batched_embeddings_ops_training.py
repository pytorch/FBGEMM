...
class SplitTableBatchedEmbeddingBagsCodegen(nn.Module):
    ...
    def ensemble_and_swap(self, ensemble_mode: Dict[str, float]) -> None:
        ...
        should_start = iter_int > int(ensemble_mode["step_start"])
        if should_ema or should_swap:
            ...
            for i in range(len(self.embedding_specs)):
                # 1) ema step
                if should_ema:
                    coef_ema = ensemble_mode["step_ema_coef"] if should_start else 0.0
                ...
                # 2) swap step
                if should_swap:
                    if should_start and int(ensemble_mode["step_mode"]) != 0:
                        weights[i].copy_(states[i][1], non_blocking=True)

        # 3) reset lr warmup
        if should_start and int(ensemble_mode["step_mode"]) != 2:
            coef_lr = (iter_int % int(ensemble_mode["step_swap"])) / float(
                ensemble_mode["step_swap"]
            )
