            ...
            for i in range(len(self.embedding_specs)):
                if should_ema:
                    ...
                    if int(ensemble_mode["step_mode"]) == 1:
                        should_ema_reset = self.iter_int % step_start == 0
                    elif int(ensemble_mode["step_mode"]) == 2:
                        should_ema_reset = self.iter_int <= step_start
                    else:
                        should_ema_reset = (self.iter_int <= step_start) or (
                            self.iter_int % step_start == 0
                        )
                    coef_ema = (
