...
class SplitTableBatchedEmbeddingBagsCodegen(nn.Module):
    ...
    def ensemble_and_swap(self, ensemble_mode: Dict[str, float]) -> None:
        """
        Perform ema, swap and re-warmup operations on the full sparse embedding tables.
        ...
        step_mode = int(ensemble_mode["step_mode"])

        should_ema = iter_int % step_ema == 0
        should_swap = iter_int % step_swap == 0
        should_start = (
            math.floor(float(iter_int) / float(step_swap)) * step_swap
            > ensemble_mode["step_start"]
        )
        ...
        if should_ema or should_swap:
            ...
            for i in range(len(self.embedding_specs)):
                # 1) ema step
                if should_ema:
                    ema_coef = ensemble_mode["step_ema_coef"] if should_start else 0.0
                ...
                # 2) swap step
                if should_swap:
                    if should_start and step_mode != 0:
                        weights[i].copy_(states[i][1], non_blocking=True)

        # 3) reset lr warmup
        if should_start and step_mode != 2:
            coef_lr = (iter_int % step_swap) / float(step_swap)
            self.set_learning_rate(
                self.optimizer_args.learning_rate * (coef_lr + 0.01 * (1.0 - coef_lr))
            )
