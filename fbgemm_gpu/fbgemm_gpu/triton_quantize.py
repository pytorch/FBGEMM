...
def triton_dequantize_mx4(a: torch.Tensor, group_size: int = 32) -> torch.Tensor:
    ...
    # TODO, consider using a different grid since most use
    # cases seem to be M=1. Maybe just arbitrarily subdivide.
    # into rows.
    grid = (M,)
...
def py_dequantize_mx4(a: torch.Tensor, group_size: int = 32) -> torch.Tensor:
    ...
    packed_input = a[:, :-1]
    shared_exp = a[:, -1:]
    # Now view as a matrix to make distribution easier.
    packed_input = torch.reshape(packed_input, [16, -1]).contiguous()
    shared_exp = torch.reshape(shared_exp, [16, -1]).contiguous()
    # Remove fp32 exponent bias
    FP32_EXP_BIAS = 127
