...
class IntNBitTableBatchedEmbeddingBagsCodegen(nn.Module):
    ...
    def __init__(  # noqa C901
        ...
        indices_dtype: torch.dtype = torch.int32,  # INT32 or INT64
    ...
    def forward(
        ...
        if indices.dtype != self.indices_dtype:
            # Some downstream code already call forward() with int64 indices,
            # so we throw warning here instead of error to prevent breaking the
            # existing code.
            warnings.warn(  # noqa: B028
                f"""\033[93m
                Indices dtype should be the same as the one specified in initialization of IntNBitTableBatchedEmbeddingBagsCodegen
                \033[0m""",
            )
