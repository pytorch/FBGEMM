...
def compute_writeback_indices(
    ...
    assert num_of_tables * max_indices < torch.iinfo(indices.dtype).max
...
def compute_writeback_indices_first_feature_only(
    ...
    Returns:
        first_indicies tensor (GPU). Empty tensor if indices is empty.
    """
    num_of_tables = len(feature_table_map)
...
def writeback_update_gradient_first_feature_only(
    ...
    grad: tuple[torch.Tensor],
    feature_table_map: list[int],
    first_indicies: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """
    ...
    Args:
        ...
        grad (tuple[torch.Tensor]): Gradient tensor to be updated
        feature_table_map (list[int]): Mapping from feature to table
        first_indicies (Optional[torch.Tensor]): Precomputed indices. If None, computed here.

    Returns:
        torch.Tensor: Updated gradient tensor with duplicates masked out
    """
    if first_indicies is None:
        first_indicies = compute_writeback_indices_first_feature_only(
            indices, offsets, feature_table_map
        )

    return writeback_apply_mask(grad, first_indicies)
