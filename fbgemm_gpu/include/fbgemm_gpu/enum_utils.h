...
#define FBGEMM_GPU_ENUM_CREATE_TAG(module_name) struct fbgemm_gpu_enum_tag_##module_name{}; \
  extern template \
  enum_registration<struct fbgemm_gpu_enum_tag_##module_name>* \
  enum_registration<struct fbgemm_gpu_enum_tag_##module_name>::registration_list;
...
enum_registration<FBGEMM_GPU_ENUM_TAG(module_name)>::registration_list = nullptr;
...
#define FBGEMM_GPU_ENUM_OP(module_name, op_name)                               \
  #op_name  "() -> ((str, (str, int)[])[])",  TORCH_FN(enum_query<FBGEMM_GPU_ENUM_TAG(module_name)>)
#define FBGEMM_GPU_ENUM_ITEM(x)  { #x, x }

using enum_item = std::tuple<std::string, int64_t>;

using enum_items = std::vector<enum_item>;

using enum_result = std::vector<std::tuple<std::string, std::vector<std::tuple<std::string, int64_t>>>>;
...
class enum_registration {
 ...
 public:
  enum_registration(const char* enum_name, enum_items&& items) : name_(enum_name) , items_(std::move(items)) {
  ...
  static enum_result enum_query() {
    ...
    for(auto next = registration_list;
        next != nullptr;
        next = next->next_) {
      result.emplace_back(std::make_tuple(std::string(next->name_), next->items_));
    }
