...
// rocprim/hip headers only compile under the actual GPU compiler (hipcc, which
// defines __HIPCC__). sparse_ops_gpu.cpp is hipified but built with the host
// clang (cxx_compile), where USE_ROCM is defined but __HIPCC__ is not, so gate
// GPU-only includes and the rocprim sort_config on __HIPCC__ to keep this
// header includable from host translation units.
#ifdef __HIPCC__
