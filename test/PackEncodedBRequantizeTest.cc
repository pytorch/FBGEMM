  ...
  for (auto shape : shapes) {
    for (int groups : {1, 3, 4}) {
      for (bool test_bias : {false, true}) {
        ...
        aligned_vector<int32_t> Cint32_buffer(Cint32_ref.size() + m * groups);
        ...
        PackEncodedBMatrix<int8_t> packedBN(
            btrans,
            k,
            n_adjusted,
            Bint8.data(),
...
TEST_P(fbgemmu8s8acc32WithQuantGranularityTest, TestFloatInputOutput) {
  ...
  for (auto shape : shapes) {
    for (int groups : {1, 3, 4}) {
      ...
      aligned_vector<int32_t> Cint32_buffer(Cfp32_ref.size() + m * groups);
...
TEST_P(fbgemmu8s8acc32Test, TestSymmetricQuantizedInputOutput) {
  ...
  for (auto shape : shapes) {
    for (int groups : {1, 3, 4}) {
      ...
      aligned_vector<int32_t> Cint32_fb_buffer(Cfp32_ref.size() + m*groups);
  ...
  for (auto shape : shapes) {
    for (int groups : {1, 3, 4}) {
      for (auto params_ptr : vec_params_ptr) {
        ...
        // stage.
        PackEncodedBMatrix<int8_t> packedWeights(
            btrans,
            k,
            n_adjusted,
            Bint8.data(),
