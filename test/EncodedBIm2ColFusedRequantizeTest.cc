...
static void Im2colTest(bool b_symmetric) {
  for (auto conv_p : shapes) {
    for (int groups : {1, 4}) {
      ...
      // assign one more column since B has one more column for encoded row sum
      // note it has to be + groups * conv_p.MB * conv_p.OUT_DIM[0] *
      // conv_p.OUT_DIM[1]
      aligned_vector<int32_t> Cint32_fb(
          Cint32_ref.size() +
          groups * conv_p.MB * conv_p.OUT_DIM[0] * conv_p.OUT_DIM[1]);
      ...
      // use PackEncodedBMatrix instead of PackBMatrix
      PackEncodedBMatrix<int8_t, ACC_T> packedB(
...
void SConvTest() {
  for (auto conv_p : shapes) {
    for (int groups : {1, 4}) {
      ...
      // use PackEncodedBMatrix instead of PackBMatrix
      PackEncodedBMatrix<int8_t, int16_t> packedB(
          matrix_op_t::NoTranspose,
          KDim,
          NDim,
          Bint8.data(),
...
static void Im2col3DTest(bool b_symmetric) {
  for (auto conv_p : shapes_3d) {
    for (int groups : {1, 4}) {
      ...
      // assign one more column since B has one more column for encoded row sum
      // note it has to be + groups * conv_p.MB * conv_p.OUT_DIM[0] *
      // conv_p.OUT_DIM[1] * conv_p.OUT_DIM[2]
      aligned_vector<int32_t> Cint32_fb(
          Cint32_ref.size() +
          groups * conv_p.MB * conv_p.OUT_DIM[0] * conv_p.OUT_DIM[1] *
              conv_p.OUT_DIM[2]);
