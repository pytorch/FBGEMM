...
void performance_test() {
  ...
  for (auto shape : shapes) {
    ...
    // need assign one more row for C_buffer because of packed encoded A has one
    // more row
...
#pragma omp parallel
#endif
      {
        ...
        // use PackEncodedAWithRowOffset instead of PackAWithRowOffset
        // to enable ABFT support
    ...
    // "Bunpacked"); printMatrix(matrix_op_t::NoTranspose, Aint8.data(), m, k,
    // k,"A unpacked"); printMatrix(matrix_op_t::NoTranspose, Cint8_local.data(),
    // m, n, n, "C requantized after"); printMatrix(matrix_op_t::NoTranspose,
    // Cint8_fb.data(), m+1, n, n, "C fb");
    // printMatrix(matrix_op_t::NoTranspose,
...
#ifdef FBGEMM_MEASURE_TIME_BREAKDOWN
    cout << setprecision(3) << setw(20) << total_packing_time / 1e6 << ", "
         << setw(20) << total_kernel_time / 1e6 << ", " << setw(20)
         << total_postprocessing_time / 1e6 << ", " << setw(20)
         << total_run_time / 1e6 << ", ";
#endif
    cout << setw(20) << runType << ", " << setw(5) << fixed << setprecision(1)
    ...
    // show execution time to measure ABFT overhead
    cout << "m, n, k: " << m << " " << n << " " << k
         << " time with abft: " << setprecision(2) << ttot / (double)1e6
         << endl;

#ifdef USE_MKL
    compare_buffers(Cint8_local.data(), Cint8_fb.data(), m, n, n, 5);
    cout << "compare buffer finished " << endl;
#endif
  }
}
